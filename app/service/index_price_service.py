...
class IndexPriceService(BaseService):
    ...
    def create_all_with_dataframe(self, dataframe: DataFrame, index: Index):
        index_prices = [
            IndexPrice(
                index.id, Price(data['Open'], data['Close'], data['High'], data['Low'], data['Change']), date.strftime('%y-%m-%d')
            )
    ...
    def is_exist_index_price(self, index_id: int, date: str) -> bool:
        try:
            self.session.query(IndexPrice).filter(IndexPrice.index_id == index_id, IndexPrice.date == date).one()
            return True
